...
# ✅ This must be FIRST before any Streamlit commands
...
if section == "🌤️ Weather Dashboard":
    ...
    if response.status_code == 200:
        ...
        today_info = data["days"][0]
        date_str = today_info["datetime"]
        import datetime
        ...
        st.metric("🌧️ Precipitation", f"{current['precip']} mm")
        st.metric("💨 Wind Speed", f"{current['windspeed']} km/h")
        st.metric("☀️ UV Index", f"{current['uvindex']}")
        st.metric("☁️ Cloud Cover", f"{current['cloudcover']}%")

        # Hourly Forecast Table
        st.subheader("🕐 Hourly Forecast")
        hourly = today_info["hours"]
        hourly_df = pd.DataFrame(hourly)[["datetime", "temp", "humidity", "precip", "windspeed", "conditions"]]
